...
// ============================================================
function getLayout(canvas, pl = 58, pr = 20, pt = 20, pb = 46) {
...
export function drawLineChart(canvas, xLabels, datasets, options = {}) {
    const ctx = setupHiDpi(canvas);
    const cw = canvas._logicalW ?? canvas.width;
    const ch = canvas._logicalH ?? canvas.height;
    ctx.clearRect(0, 0, cw, ch);
    ctx.fillStyle = '#0c1530';
    ctx.fillRect(0, 0, cw, ch);
    const l = getLayout(canvas);
    // override canvas dims for layout
    const layout = { left: 58, right: 20, top: 24, bottom: 46, w: cw - 58 - 20, h: ch - 24 - 46 };
    ...
    for (let g = 0; g <= 5; g++) {
        ...
        ctx.lineTo(layout.left + layout.w, y);
        ctx.stroke();
...
export function drawStackedAreaChart(canvas, xLabels, datasets, options = {}) {
    const ctx = setupHiDpi(canvas);
    const cw = canvas._logicalW ?? canvas.width;
    const ch = canvas._logicalH ?? canvas.height;
    ctx.clearRect(0, 0, cw, ch);
