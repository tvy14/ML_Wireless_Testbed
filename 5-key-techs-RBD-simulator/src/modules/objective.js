...
export function initObjective() {
    const canvas = document.getElementById('obj-chart');
    const alphaSlider = document.getElementById('obj-alpha');
    const betaSlider = document.getElementById('obj-beta');
    const perfDisp = document.getElementById('obj-perf');
    const alphaVal = document.getElementById('obj-alpha-val');
    const betaVal = document.getElementById('obj-beta-val');
    const perfVal = document.getElementById('obj-perf-val');
    const wSliders = [0, 1, 2, 3, 4].map(i => document.getElementById(`obj-w${i}`));
    const wVals = [0, 1, 2, 3, 4].map(i => document.getElementById(`obj-w${i}-val`));
    const weightNote = document.getElementById('weight-sum-note');
    const metricsDiv = document.getElementById('obj-metrics');
    function getWeights() {
    ...
    function render() {
        ...
        // per-tech Sp_i(t)
        const spSeries = TECHS.map(tech => TIME_AXIS.map(t => sp(tech, t, alpha, beta)));
        // SRI(t) = Σ w_i * Sp_i(t)  [normalized weights]
        const sriData = TIME_AXIS.map((_, ti) => TECHS.reduce((s, _tech, i) => s + wNorm[i] * spSeries[i][ti], 0));
