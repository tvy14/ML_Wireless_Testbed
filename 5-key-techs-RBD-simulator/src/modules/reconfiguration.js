...
function reconfig(rSys, gamma, rMin) {
    return rSys < rMin ? rSys + (1 - rSys) * gamma : rSys;
}
export function initReconfiguration() {
    const canvas = document.getElementById('p3-chart');
    const techSelect = document.getElementById('p3-tech-select');
    const metricsDiv = document.getElementById('p3-metrics');
    function render() {
        const sel = techSelect.value;
        const datasets = [];
        // pre-compute for all
        const allOriginal = TECHS.map(tech => TIME_AXIS.map(t => systemReliability(tech, t)));
        const allReconfig = TECHS.map((tech, i) => allOriginal[i].map(r => reconfig(r, tech.gamma, tech.rMin)));
        if (sel === 'all') {
            // Show SRI(t) before and after reconfiguration
            const sriOriginal = TIME_AXIS.map((_, ti) => TECHS.reduce((s, tech, i) => s + tech.sriWeight * allOriginal[i][ti], 0));
            const sriReconfig = TIME_AXIS.map((_, ti) => TECHS.reduce((s, tech, i) => s + tech.sriWeight * allReconfig[i][ti], 0));
            datasets.push({
